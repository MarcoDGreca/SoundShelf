    ...
    <div class="add-product-page-container">
        ...
        <form action="<%= request.getContextPath() %>/ModificaProdottoControl" method="post" onsubmit="return confirmEdit();">
            <fieldset class="add-product-fieldset">
                <legend class="add-product-legend">Modifica Dettagli del Prodotto</legend>
                ...
                <div class="add-product-form-group">
                    ...
                    <input type="text" id="name" name="name" class="add-product-input" value="<%= product.getName() %>" required><br>
                </div>

                <div class="add-product-form-group">
                    <label for="description" class="add-product-label">Descrizione:</label>
                    <textarea id="description" name="description" class="add-product-textarea" required><%= product.getDescription() %></textarea><br>
                </div>

                <div class="add-product-form-group">
                    <label for="salePrice" class="add-product-label">Prezzo di Vendita (&euro;):</label>
                    <input type="number" step="0.01" id="salePrice" name="salePrice" class="add-product-input" value="<%= product.getSalePrice() %>" required><br>
                </div>

                <div class="add-product-form-group">
                    <label for="originalPrice" class="add-product-label">Prezzo Originale (&euro;):</label>
                    <input type="number" step="0.01" id="originalPrice" name="originalPrice" class="add-product-input" value="<%= product.getOriginalPrice() %>" required><br>
                </div>

                <div class="add-product-form-group">
                    <label for="availability" class="add-product-label">Disponibilit&aacute;:</label>
                    <input type="number" id="availability" name="availability" class="add-product-input" value="<%= product.getAvailability() %>" required><br>
                </div>

                <div class="add-product-form-group">
                    <label for="releaseDate" class="add-product-label">Data di Rilascio:</label>
                    <input type="date" id="releaseDate" name="releaseDate" class="add-product-input" value="<%= product.getReleaseDate() %>" required><br>
                </div>

                <div class="add-product-form-group">
                    <label for="image" class="add-product-label">Immagine URL:</label>
                    <input type="text" id="image" name="image" class="add-product-input" value="<%= product.getImage() %>" required><br>
                </div>

                <div class="add-product-form-group">
                    <label for="supportedDevice" class="add-product-label">Formato:</label>
                    <input type="text" id="supportedDevice" name="supportedDevice" class="add-product-input" value="<%= product.getSupportedDevice() %>" required><br>
                </div>

                <div class="add-product-form-group">
                    <button type="submit" class="add-product-submit-btn">Salva Modifiche</button>
                </div>
            </fieldset>
        </form>
        <% 
